...
_2026-04-28。合并自 `training_modeller_zh.md`（2026-04-23 架构审查）与 `training_modeller_zh_v2.md`（2026-04-28 统一方案）。路线图实施状态同步至 2026-04-28。_
...
| `TrainingReport` 统一（Phase 0） | `python/zrt/training/spec/report.py` | ✅ 两路径均返回 `TrainingReport` |
| `OpGraph.from_model_spec()` 工厂方法（Phase 1） | `python/zrt/ir/graph.py` | ✅ 已实现 |
| 跨路径导入别名清理（Phase 2） | `python/zrt/transform/analysis/training.py` | ✅ 已清理 |
| `--quant` CLI 参数 + `QuantConfig` 注入 | `python/zrt/cli.py` | ✅ 量化分析路径可用 |
| `_effective_compute_dtype()` + `RooflinePass` 修正 | `python/zrt/transform/analysis/passes.py` | ✅ 量化节点 peak TFLOPS 正确 |
...
| 统一项 | 达成状态 ✅ |
|--------|---------|
| 输出类型 | ✅ 两者均返回 `TrainingReport` |
| 合成 OpGraph | ✅ `OpGraph.from_model_spec()` 已实现 |
| 跨路径类型泄漏 | ✅ 下划线别名已清理 |
...
**A.1 — 步骤时间：PP 平均化回退** ✅（`training.py:471–553`）
...
- **实现**：`stage_id` 存在时逐阶段调用 `DAGScheduler` + `phase_latency("fwd"/"bwd")`；回退路径保留并发出 warning。

**A.2 — 激活内存：退化系数路径** ✅（`training.py:234–291`）
...
- **实现**：`fwd_bwd_stitched=True` 时走图原生路径；Korthikanti 回退已整合 CP 分片（`/cp`）、recompute 注解推导、ZeRO 因子。

**A.3 — 总 FLOPs：覆盖逻辑** ✅（`training.py:24–113`）

`6P` 兜底估算有时覆盖 `FlopsPass` 已正确标注的逐节点 `flops_fwd/dx/dw`。
- **修复**：对所有节点求和逐节点 FLOPs；删除覆盖逻辑；仅在 `FlopsPass` 未运行时将 `6P` 作为回退。
- **实现**：逐节点求和优先；仅当 `forward_flops == 0 and backward_flops == 0` 时启用 `6P` 回退。

---

### 阶段 B — 流水线并行阶段分配与合成 ✅ 已完成

**B.1 — `PipelineParallelPass` 完善** ✅（`transform/parallel/pipeline_parallel.py:39–249`）
...
- 负载回退链：`compute_us` → `latency_us` → `flops/1e12` → `1.0`；阶段不均衡时发出 warning。

**B.2 — 逐阶段 DAGScheduler 调度** ✅（`training.py:407–469`）
...
```
EP 不均衡因子在此阶段应用于各 stage 时延。

**B.3 — 1F1B 合成器修正** ✅（`schedules.py:63–134`）
```
...
```
使用真实阶段时延，而非基于计数的公式。DP overlap 在 bubble 窗口内正确扣除。

---

### 阶段 C — 并行度完整性与精确内存建模 ✅ 已完成

**C.1 — `ContextParallelPass`** ✅（`context_parallel.py:8–98` 注解 + `comm_inserter.py:182–269` 插入）
- **Ulysses CP**：在注意力前插入 `comm.all_to_all`（分散序列、聚合头），在注意力后插入逆变换。组大小 = `cp`，数据量 = `b × s/cp × h`。
- **Ring CP**：在注意力内插入 `cp` 轮 `comm.send_recv`，每轮对应一个 KV 块。标记为可与 FA tile 计算重叠（`annotations["overlap_target"] = "fa_tile:<node_id>"`）。
- 通信实际插入由 `CommInserterPass._insert_cp_comm()` 完成（`ContextParallelPass` 仅写注解）。

**C.2 — `DataParallelPass`** ✅（`data_parallel.py:18–188`）
...
**C.3 — `StreamAssignPass` 中的 CoC/MC2 重叠规则** ✅（`passes.py:190–257` 检测；`training.py:733–762` 计算暴露时间）
...
**C.4 — 基于图的精确内存建模** ✅（`training.py:142–362 TrainingMemoryPass`）
...
**D.1 — VPP / 交错 1F1B** ✅（`schedules.py:137–188 InterleavedComposer`）
bubble 公式：`(pp-1) / (vpp × M)`。`vpp_chunks` 从 `Strategy` 读取；V≤1 时回退到 `OneF1BComposer`。

**D.2 — DualPipe / DualPipeV** ✅（`schedules.py:191–285`）
每个阶段并发运行 μbatch_i 前向与 μbatch_{i-1} 反向；DualPipe bubble ≈ `(pp-1)/2 · t_stage`；DualPipeV 进一步缩小至 `(pp-1)/(2V) · t_stage`。两者均支持 DP overlap 扣除。

**D.3 — EP 负载不均衡** ⚠️ 启发式（`training.py:700–728 _detect_ep_imbalance`）
`t_expert_bottleneck = t_avg * (1 + imbalance)`，作为乘数应用于专家矩阵乘节点时延。
当前使用静态查表 `{2: 1.05, 4: 1.10, 8: 1.15, 16: 1.20}`，待接入真实 token 路由统计后替换。

**D.4 — 搜索与帕累托前沿** ✅（`search/estimator.py:71–144`，非 `sweep.py`）
`grid_search()` 对 `(tp, cp, pp, ep, dp, zero_stage)` 六维网格搜索，附带剪枝规则，返回按 `step_time_ms` 排序的 `TrainingReport` 列表；`pareto_frontier()` 输出按 `(step_time, peak_hbm)` 的帕累托前沿。`SearchSpace` 支持 `pp_schedules`、`recompute_policies`、`vpp_chunks_values` 等维度。

**D.5 — 锚点验证** ✅（`tests/training/anchors/`）
GPT-3 175B、Llama-3 70B、DeepSeek-V3 三套 YAML fixture —— CI 断言估算 MFU 误差在 10–15% 以内。
...
以下问题已记录但尚未实施修复，见 `docs/architecture_overview_zh.md` §7 对应条目。

| # | 问题 | 所属文件 | 优先级 |
|---|------|---------|-------|
| 1 | `OffloadPass` 已完整实现但未在 `pipeline.py` 的 `build_pipeline()` 中注册，训练流水线不会执行 offload 建模 | `transform/pipeline.py` | 中 |
| 2 | 训练 IR MoE/MTP 回退到 dense_block（Stack A FLOPs 不准，Phase 2 占位注释） | `training/ir/builders.py` | 中 |
| 3 | Stack A sharding 仅 TP，`ShardPlan` 已读取 CP/EP 字段但不插入对应集合（Phase 2 待补） | `training/ir/shard.py` | 低 |
| 4 | `perf_tables.py` 简易启发，`gpu_name`/`dtype` 参数未参与逻辑（仅按规模分档） | `training/io/perf_tables.py` | 低 |
| 5 | `OpenBoxModel`/`OperatorOptimizationModel`/`SystemDesignModel` 均为 stub（`predict()` 为空） | `policy_model/` | 低（设施就绪前保持开放） |
| 6 | `LookupSimulator`/`TilesimSimulator` stub，`can_simulate→False`；`RooflineSimulator` 为唯一有效后端 | `simulator/backends/` | 低（设施就绪前保持开放） |
| 7 | EP 负载不均衡系数为静态查表启发式（EP 2→1.05 … 16→1.20），未接入真实 token 路由统计 | `transform/analysis/training.py` | 低 |

**已解决（已从上表移除）**：
- ~~`RecomputePass`/`OptimizerPass` 未注册~~：两者均已在 `pipeline.py:111/114` 注册 ✅
- ~~PP>1 无 `stage_id` 时按 pp 平均~~：已修复，逐阶段 DAGScheduler + fallback warning ✅
- ~~`6P` 兜底覆盖逐节点 FLOPs~~：已修复，仅 `forward_flops==0` 时回退 ✅
